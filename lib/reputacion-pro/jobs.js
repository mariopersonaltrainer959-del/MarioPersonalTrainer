...
async function processDueJobs() {
  ...
  for (const job of jobs) {
    try {
      ...
      const inCooldown = await hasRecentReviewEmail(job.negocio_id, cita.paciente_id, REVIEW_COOLDOWN_DAYS);
      if (inCooldown) {
        // Evita fatigar al cliente: no reenviar review dentro de la ventana de cooldown.
        await markJobSent(job.id);
        continue;
      }
...
async function hasRecentReviewEmail(negocioId, pacienteId, days) {
  ...
  const row = await getQuery(
    `SELECT rr.id
     ...
     JOIN citas c ON c.id = rr.session_id
     WHERE rr.professional_id = ?
       AND rr.email_enviado = 1
       AND c.paciente_id = ?
       AND rr.fecha >= ?
     ORDER BY rr.fecha DESC
     LIMIT 1`,
    [negocioId, pacienteId, cutoff]
  ).catch(() => null);
  return !!row;
}
