...
async function getSessionForFeedback(sessionId) {
  if (String(sessionId).trim().toLowerCase() === 'demo') {
    const negocio = await negocioService.getById(1).catch(() => null);
    const googleReviewUrl = negocio && negocio.google_review_url ? negocio.google_review_url : null;
    return { valid: true, googleReviewUrl, negocioId: 1, isDemo: true };
  }
...
async function hasExistingFeedback(sessionId) {
  if (String(sessionId).trim().toLowerCase() === 'demo') return false;
...
async function submitRating(negocioId, sessionId, rating, comentario = null) {
  if (String(sessionId).trim().toLowerCase() === 'demo') return;
  const id = parseInt(sessionId, 10);
