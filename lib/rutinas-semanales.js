...
/** Paleta alineada con landing-theme.css (dorado + negro) */
const PDF_THEME = {
  gold: '#c9a84c',
  goldLight: '#e8c97a',
  goldDark: '#9a7232',
...
function drawPdfFooter(doc, contacto, pageBottom) {
  const barH = 48;
  const y = pageBottom - barH;
  doc.fillColor(PDF_THEME.black).rect(0, y, 595.28, barH).fill();
  doc.fillColor(PDF_THEME.gold).rect(0, y, 595.28, 2).fill();
  doc.fontSize(8).fillColor(PDF_THEME.goldLight);
  const linea = contacto.filter(Boolean).join('  ·  ');
  if (linea) {
    doc.text(linea, 48, y + 12, { width: 499, align: 'center' });
  }
  doc.fillColor(PDF_THEME.textMuted).text('Plan personalizado · Confidencial', 48, y + 26, { width: 499, align: 'center' });
}

function drawPdfHeader(doc, nombreNegocio) {
  const T = PDF_THEME;
  doc.fillColor(T.black).rect(0, 0, 595.28, 112).fill();
  doc.fillColor(T.darkMid).rect(0, 0, 595.28, 112).fill();
  doc.fillColor(T.gold).rect(0, 108, 595.28, 4).fill();
  doc.fillColor(T.white).fontSize(26).text('Plan de entrenamiento', 48, 36, { width: 500 });
  doc.fontSize(11).fillColor(T.goldLight).text(nombreNegocio || 'Entrenador personal', 48, 72, { width: 500 });
}
...
function drawClientCard(doc, paciente, semanaInicio, semanaFin, y) {
  ...
  const w = 499;
  const h = 76;
  fillRoundRect(doc, x, y, w, h, 8, T.darkMid);
  strokeRoundRect(doc, x, y, w, h, 8, T.goldDark, 0.5);
  doc.fillColor(T.gold).rect(x, y, 5, h).fill();
  doc.fillColor(T.gold).fontSize(8).text('PREPARADO PARA', x + 20, y + 16);
  doc.fillColor(T.white).fontSize(18).text(paciente.nombre || '—', x + 20, y + 30, { width: w - 40 });
  doc.fillColor(T.goldLight).fontSize(10).text(
...
function drawDayBlock(doc, diaLabel, ejercicios, y, contentX, contentW, alt) {
  ...
  const bg = alt ? T.cream : T.white;
  fillRoundRect(doc, contentX, y, contentW, blockH, 6, bg);
  strokeRoundRect(doc, contentX, y, contentW, blockH, 6, T.border);

  const pillW = 88;
  fillRoundRect(doc, contentX + 12, y + 12, pillW, 22, 11, T.gold);
  doc.fillColor(T.black).fontSize(10).text(diaLabel, contentX + 12, y + 17, { width: pillW, align: 'center' });
  ...
  if (!ejercicios.length) {
    doc.fontSize(10).fillColor(T.textMuted).text('Descanso o sin ejercicios programados', ex, ey, { width: ew });
    ey += 20;
  } else {
    doc.fontSize(10);
    for (const ej of ejercicios) {
      const lineH = doc.heightOfString(ej, { width: ew - 16 });
      doc.fillColor(T.gold).circle(ex + 4, ey + 5, 2.5).fill();
      doc.fillColor(T.text).text(ej, ex + 14, ey, { width: ew - 16 });
      ey += lineH + 10;
    }
  }
...
function drawNotesBlock(doc, notas, y, contentX, contentW) {
  ...
  fillRoundRect(doc, contentX, y, contentW, blockH, 6, T.dark);
  strokeRoundRect(doc, contentX, y, contentW, blockH, 6, T.gold, 1);
  doc.fillColor(T.goldLight).fontSize(9).text('NOTAS DEL ENTRENADOR', contentX + pad, y + 14);
  doc.fillColor(T.textOnDark).fontSize(10).text(notas, contentX + pad, y + 32, { width: contentW - pad * 2 });
...
async function generatePdfBuffer(negocioId, pacienteId, semanaInicio) {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.fillColor(PDF_THEME.goldDark).fontSize(9).text('TU RUTINA SEMANAL', contentX, y);
    y += 22;
