...
/** totalBruto = base + IVA; IRPF se calcula sobre la base imponible */
function calcularImportesFactura(totalBruto, ivaPct, irpfPct) {
  const total = parseFloat(totalBruto) || 0;
  const iva = parseFloat(ivaPct);
  const ivaNum = Number.isFinite(iva) ? iva : 21;
  const irpf = parseFloat(irpfPct);
  const irpfNum = Number.isFinite(irpf) ? Math.max(0, irpf) : 0;
  const precioBase = Math.round((total / (1 + ivaNum / 100)) * 100) / 100;
  const ivaEur = Math.round((total - precioBase) * 100) / 100;
  const irpfEur = irpfNum > 0 ? Math.round((precioBase * irpfNum / 100) * 100) / 100 : 0;
  const liquido = Math.round((total - irpfEur) * 100) / 100;
  return { precioBase, ivaPct: ivaNum, ivaEur, irpfPct: irpfNum, irpfEur, total, liquido };
}
...
async function create(negocioId, data) {
  const {
    ...
    concepto, descripcion, totalPagado, ivaPct, irpfPct, forma_pago
  } = data;
  const { precioBase, ivaPct: ivaPctNum, ivaEur, irpfPct: irpfPctNum, irpfEur, total } = calcularImportesFactura(
    totalPagado,
    ivaPct,
    irpfPct != null && irpfPct !== '' ? irpfPct : 7
  );
  const numero = await getNextNumeroFactura(negocioId);
  const fecha = new Date().toISOString().slice(0, 10);
  await runQuery(
    `INSERT INTO facturas (negocio_id, numero_factura, fecha_emision, cliente_nombre, cliente_nif, cliente_direccion, cliente_cp, cliente_ciudad, cliente_provincia, concepto, descripcion, precio_base, iva_pct, iva_eur, irpf_pct, irpf_eur, total, forma_pago)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      ...
      precioBase, ivaPctNum, ivaEur, irpfPctNum, irpfEur, total,
...
async function generatePdfBuffer(negocioId, facturaId) {
  ...
  await new Promise((resolve, reject) => {
    ...
    y += lineHeight;
    const irpfPct = parseFloat(factura.irpf_pct) || 0;
    const irpfEur = parseFloat(factura.irpf_eur) || 0;
    if (irpfPct > 0 && irpfEur > 0) {
      doc.text(`Retención IRPF (${irpfPct}%): -${irpfEur.toFixed(2).replace('.', ',')} €`, 400, y);
      y += lineHeight;
    }
    doc.fontSize(11).fillColor(blue).text('TOTAL FACTURA: ' + formatEuro(factura.total), 400, y);
    y += lineHeight;
    if (irpfPct > 0 && irpfEur > 0) {
      const liquido = Math.round((parseFloat(factura.total) - irpfEur) * 100) / 100;
      doc.fontSize(10).fillColor('#374151').text('Total a ingresar (líquido): ' + formatEuro(liquido), 400, y);
      y += lineHeight;
    }
    y += blockGap + 10;
