...
/**
 * Renderiza asunto y cuerpo con variables.
 * Variables comunes: {{nombre_cliente}}, {{nombre_paciente}}, {{fecha}}, {{hora}}, {{nombre_negocio}}, {{feedback_url}}
 */
function render(plantilla, inputVars = {}) {
  const vars = {
    nombre_cliente: '',
    nombre_paciente: '',
    fecha: '',
  ...
  if (!vars.nombre_cliente && vars.nombre_paciente) vars.nombre_cliente = vars.nombre_paciente;
  if (!vars.nombre_paciente && vars.nombre_cliente) vars.nombre_paciente = vars.nombre_cliente;
  return {
